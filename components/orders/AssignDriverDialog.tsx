...
import { fetchDrivers, type DriverLite } from "@/lib/operations";
import {
  assignDriversBulk,
  type OrderTaskType,
} from "@/lib/orders";
...
export default function AssignDriverDialog({
  ...
  const primaryWarehouseId = React.useMemo(() => getPrimaryWarehouseId(authUser), [authUser]);
  ...
  const mutation = useMutation({
    mutationFn: async (printAfterAssign: boolean) => {
      ...
      await printDriverManifest({
        ...
        warehouseName: primaryWarehouseId ?? selectedDriver.warehouseId,
        locale,
          ...
          <div className="space-y-2">
            ...
            {driversQuery.isLoading ? (
              ...
              <div className="text-sm text-muted-foreground">
                No drivers found. Create a driver user and assign driver permissions.
              </div>
            ) : (
              <select
                value={driverId}
